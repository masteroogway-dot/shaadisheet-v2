...
function buildKeyframes(from: Record<string, unknown>, steps: Record<string, unknown>[]) {
  const keys = new Set([...Object.keys(from), ...steps.flatMap((s) => Object.keys(s))]);
  const keyframes: Record<string, unknown[]> = {};
  keys.forEach((k) => {
    keyframes[k] = [from[k], ...steps.map((s) => s[k])];
  });
  return keyframes as Record<string, (string | number)[]>;
}
...
export default function BlurText({
  ...
  delay = 200,
  className = "",
  animateBy = "words" as "words" | "chars",
