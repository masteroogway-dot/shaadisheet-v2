...
function ShareableCard({ name1, name2, hashtags }: { name1: string; name2: string; hashtags: string[] }) {
  return (
    <div
      ...
      className="w-[600px] p-10 text-center relative overflow-hidden"
      style={{
                ...
                >
                  ...
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                    <motion.button
                      ...
                      whileTap={{ scale: 0.9 }}
                      className="px-2.5 py-1 text-xs text-gray-600 hover:bg-gray-100 rounded-md font-medium"
                    >
                      {copiedId === h.id ? "✓ Copied" : "Copy"}
                    </motion.button>
                    {canEdit && (
                      <button onClick={() => handleDelete(h.id)} className="px-2.5 py-1 text-xs text-red-500 hover:bg-red-50 rounded-md font-medium">Delete</button>
                    )}
                  </div>
                </motion.div>
              );
